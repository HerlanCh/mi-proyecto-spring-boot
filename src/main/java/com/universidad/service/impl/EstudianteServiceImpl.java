...
public class EstudianteServiceImpl implements IEstudianteService { // Define la clase EstudianteServiceImpl que implementa la interfaz IEstudianteService
    ...
    public EstudianteDTO registrarEstudiante(EstudianteDTO estudianteDTO) {
        Estudiante estudiante = convertToEntity(estudianteDTO); // Convertir DTO a entidad
        Estudiante estudianteGuardado = estudianteRepository.save(estudiante); // Guardar en el repositorio
        return convertToDTO(estudianteGuardado); // Convertir entidad a DTO y devolver
    }
    ...
    public EstudianteDTO actualizarEstudiante(Long id, EstudianteDTO estudianteDTO) {
        ...
        // Actualizar los campos del estudiante existente con los nuevos valores
        ...
        // Convertir la entidad actualizada a DTO y retornarlo
        return convertToDTO(estudianteExistente);
    }

    @Override
    public void eliminarEstudiante(Long id) {
        if (!estudianteRepository.existePorId(id)) {
            throw new RuntimeException("Estudiante no encontrado");
        }
